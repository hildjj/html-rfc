    ...
    jsdom.env({
        html: html,
            ...
            window.$.fn.extend({
                ...
                textNodes: function() {
                    return window.$(window.$.map(this, function(n) {
                        return window.$.grep(n.childNodes, function(n) {
                            return n.nodeType == 3;
                        });
                    }));
                }
            ...
            var tiny_path = path.resolve(path.dirname(module.filename),
                                         "jquery.tinysort.js");
            fs.readFile(tiny_path, 'utf-8', function(err, data) {
                if (err) {
                    def.reject(err);
                } else {
                    var jQuery = that.window.$;
                    eval(data);
                    def.resolve(window);
                }
            });
...
RFC.prototype.lint = function(nits) {
    var that = this;
    if (!nits) {
        nits = this.nits;
    }
    nits.forEach(function(n){
        n.reset();
    });
    that.current_nit = null;
    return whenEver(nits, function(n) {
        return n.exec(that);
    });
};
